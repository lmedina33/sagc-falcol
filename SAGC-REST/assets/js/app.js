...
$(document).ready(function() {
    ...
    $('#logout a').click(function(e) {
        ...
        // ask verification
        $.SmartMessageBox({
            title: "<i class='fa fa-sign-out txt-color-orangeDark'></i> Sair do sistema",
...
if ($.navAsAjax)
{
    ...
    $(document).on('click', 'a[href!="#"]', function(e) {
        ...
        // marcelito
        $this.attr("href", $this.attr("href").replace(window.location.href.replace(window.location.hash, "").replace(window.location.search, ""), ""));

        // if parent is not active then get hash, or else page is assumed to be loaded
        if (!$this.parent().is("nav .active") && !$this.attr('target')) {
...
function checkURL() {
    ...
    if (url) {
        ...
        // marcelito
        loadURL(url + "?navAsAjax=1", container);
...
function sucessDialogAlert(titulo, mensagem) {
     dialogAlert(titulo,mensagem,"#739E73","fa fa-check",20000);
}

function erroDialogAlert(titulo, mensagem) {
     dialogAlert(titulo,mensagem,"#C46A69","fa fa-warning shake animated",20000);
}